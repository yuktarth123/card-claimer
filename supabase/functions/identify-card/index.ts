...
- "number": the card's own position-in-set number, usually bottom-left, in "position/total" form like "4/102" or "004/091", else null. This is the number on THIS physical print and may differ from the English release's number -- report exactly what's printed, don't translate or guess the English equivalent. Small Chinese Gem Pack / promo sets often print a SEPARATE pack or print-run code right next to this (e.g. a "19" next to "06/07") -- that code is not part of the card number. Report only the position/total pair itself (e.g. "06/07"), never a concatenation of the two (do not report something like "1906/07" or "19/06/07"). If you can only find one of the two numbers and aren't sure which it is, report just that number rather than guessing a combined value.
